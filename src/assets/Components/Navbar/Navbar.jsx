            ...
            <nav className="flex justify-between items-center container mx-auto py-6">
                ...
                <div className="max-md:hidden">
                    <ul className="flex gap-2">
                        <Link onClick={() => setPath(0)} to="/" className={`px-3 py-2 font-medium ${isRoot ? "text-white" : ""}`}>Home</Link>
                        <Link onClick={() => setPath(1)} to="statistics" className={`${path === 1 ? 'text-purple-600' : ''} px-3 py-2 font-medium ${isRoot ? "text-white" : ""}`}>Statistics</Link>
                        <Link onClick={() => setPath(2)} to="dashboard" className={`${path === 2 ? 'text-purple-600' : ''} px-3 py-2 font-medium ${isRoot ? "text-white" : ""}`}>Dashboard</Link>
                        <Link onClick={() => setPath(3)} to="purchased" className={`${path === 3 ? 'text-purple-600' : ''} px-3 py-2 font-medium ${isRoot ? "text-white" : ""}`}>Purchased</Link>
                    </ul>
                </div>
                {/* nav button part */}
                <div className="flex gap-4 items-center">
                    <div className="relative hidden max-md:flex">
                        <FaBarsStaggered onClick={toggleNavbar} className={`cursor-pointer text-2xl ${isRoot ? 'text-white' : 'text-gray-800'}`} />
                        <ul className={`absolute right-10 bg-gray-600 flex-col ${isOpen ? 'flex' : 'hidden'} text-white`}>
